...
export const CustomerGroupCreateSchema = z
  .object({
    ...
    points_multiplier: z.coerce.number().min(0).max(100).optional().default(1),
...
export const CustomerGroupUpdateSchema = CustomerGroupCreateSchema.partial().openapi(
...
export const CustomerTagUpdateSchema = CustomerTagCreateSchema.partial().openapi(
...
registry.registerPath({
  ...
  description: 'Export semua pelanggan ke CSV. Authorization required, MIME text/csv.',
  tags: ['Customers'],
  security: [{ bearerAuth: [] }],
  responses: {
    200: {
...
registry.registerPath({
  ...
  description: 'Import pelanggan dari array baris CSV (sudah diparse di client).',
  tags: ['Customers'],
  security: [{ bearerAuth: [] }],
  request: {
    body: {

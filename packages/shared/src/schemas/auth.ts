...
export const UserRoleSchema = z.enum(['admin', 'cashier']).openapi({ example: 'admin' });
...
export type Login2FARequiredResponse = z.infer<typeof Login2FARequiredResponseSchema>;
...
export const RefreshResponseSchema = LoginResponseSchema.openapi('RefreshResponse');
...
registry.registerPath({
  ...
  description: 'Generate reset link. Untuk dev environment, link di-print ke server console.',
  tags: ['Auth'],
  request: {
    body: {
      required: true,
...
registry.registerPath({
  ...
  description: 'Verify TOTP code yang user input dari authenticator app, lalu enable 2FA.',
  tags: ['Auth'],
  security: [{ [bearerAuth.name]: [] }],
  request: {
    body: {

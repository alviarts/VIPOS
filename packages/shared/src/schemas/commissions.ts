...
export const CommissionAmountBasisSchema = z.enum(['PER_TRANSACTION', 'PER_ITEM']).openapi({
  description:
    'PER_TRANSACTION: amount × jumlah transaksi qualifying. PER_ITEM: amount × qty produk qualifying.',
});

export const CommissionAppliesToScopeSchema = z.enum(['all', 'roles', 'employees']).openapi({
  description:
...
export const CommissionCalcPeriodSchema = z.enum(['DAY', 'WEEK', 'MONTH']).openapi({
  description: 'Periode kalkulasi untuk TIERED (cumulative basis di-reset tiap periode).',
});
...
export const CommissionGroupSchema = z
  .object({
    ...
    applies_to_category_ids: z.array(z.number().int().positive()).nullable().optional(),
    applies_to_product_ids: z.array(z.number().int().positive()).nullable().optional(),
...
const baseGroupCreate = z.object({
  ...
  applies_to_employee_ids: z.array(z.number().int().positive()).optional().nullable(),
  applies_to_products_scope: CommissionAppliesToProductsScopeSchema.default('all'),
  applies_to_category_ids: z.array(z.number().int().positive()).optional().nullable(),
  applies_to_product_ids: z.array(z.number().int().positive()).optional().nullable(),
...
export const CommissionGroupCreateSchema = baseGroupCreate.openapi('CommissionGroupCreate');
...
export const CommissionAssignmentCreateSchema = z
  .object({
    transaction_id: z.number().int().positive(),
    employee_id: z.number().int().positive(),
    commission_group_ids: z.array(z.number().int().positive()).optional().openapi({
      description: 'Optional. Kalau kosong, backend pilih semua group active yang qualifying.',
    }),
    notes: z.string().max(255).optional().nullable(),
  })
  .openapi('CommissionAssignmentCreate');
export type CommissionAssignmentCreate = z.infer<typeof CommissionAssignmentCreateSchema>;

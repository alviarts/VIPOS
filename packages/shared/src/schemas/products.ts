...
const intFromForm = z
  ...
  .transform((v, ctx) => {
    ...
    if (!Number.isFinite(n)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Bukan integer valid' });
      return z.NEVER;
    }
    return n;
  });
...
const boolIntFromForm = z
  .union([z.boolean(), z.literal(0), z.literal(1), z.literal('0'), z.literal('1'), z.literal('true'), z.literal('false')])
...
export const ProductCreateSchema = z
  .object({
    ...
    price: numFromForm.refine((v) => v !== null && v >= 0, { message: 'Harga jual wajib diisi dan >= 0' }),
    ...
    image_url: z.string().url('URL gambar tidak valid').max(2048).optional().nullable().or(z.literal('').transform(() => null)),

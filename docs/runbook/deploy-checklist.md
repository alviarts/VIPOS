...
| 1b  | Backup-freshness endpoint                                                                                              | `curl -fsS https://<host>/api/health/backup` → `{"status":"ok"}`. 503 + `status:stale` is OK on a brand-new deploy with no dumps yet; once the daily 02:00 UTC cron has run, expect 200.   |
| 1c  | Disk-usage endpoint                                                                                                    | `curl -fsS https://<host>/api/health/disk` → `{"status":"ok"}`. 503 + `status:high_usage` means the backup volume is ≥90% full; clear `dist.pre-deploy-*` snapshots or rotate older dumps. |

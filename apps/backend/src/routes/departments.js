...
router.delete('/:id', authenticateToken, requireAdmin, (req, res) => {
  try {
    const db = getDb();
    const used = db.prepare('SELECT COUNT(*) as count FROM categories WHERE department_id = ?').get(req.params.id);
    if (used.count > 0) {
      return res.status(400).json({ error: 'Departemen masih dipakai oleh kategori. Pindahkan kategori dulu.' });
    }
